...
# class CrawlerSpider(Spider):
#     name = "DataJapanese"
#     allowed_domains = ["tiengnhatcse.wordpress.com"]
#     start_urls = [
#         "https://tiengnhatcse.wordpress.com/2016/03/05/kiem-tra-bai-2-minna/",
#     ]

#     def parse(self, response):
#         questions = Selector(response).xpath(
#             '//div[@class="entry-content post_content"]/p')
#         k = 219
#         i = 1
#         for a in questions:
#             item = answer()
#             item['question_id'] = k
#             item['content'] = a.extract()
#             item['is_true'] = 0
#             i = i + 1
#             if(i==4):
#                 k = k + 1
...
class CrawlerSpider(Spider):
    name = "DataJapanese"
    allowed_domains = ["nguphaptiengnhat.net"]
    start_urls = [
        "https://nguphaptiengnhat.net/test/n4/de-thi-tieng-nhat-n5-de-so-6-2.html#comment-5",
    ]

    def parse(self, response):
        questions = Selector(response).xpath(
            '//div[@class="wpProQuiz_quiz"]/ol/li')
        k = 238
        for a in questions:
            b = a.xpath('div[@class="wpProQuiz_question"]/ul/li')
            k = k + 1
            for i in b:
                item = answer()
                item['question_id'] = k
                item['content'] = i.xpath('label/text()').extract()
                item['is_true'] = 0
                yield item
